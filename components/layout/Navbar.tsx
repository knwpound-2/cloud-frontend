...
import { Home, History, Settings, Wind } from "lucide-react";
        ...
        <div className="flex items-center gap-2 bg-white/50 backdrop-blur-md p-1.5 rounded-2xl border border-white/50 shadow-sm">
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-slate-600 font-medium text-sm transition-all hover:bg-white hover:text-orange-600 hover:shadow-sm active:scale-95"
            >
              {item.icon}
